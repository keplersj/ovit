...
static enum { accAUTO,
			  accDIRECT,
			  accKERNEL } tivo_partition_accmode = accAUTO;
...
file_or_dev_size(int fd, uint64_t *size)
{
...
#ifdef DKIOCGETBLOCKCOUNT /* For Mac OS X */
	{
		...
		{
			if ((sizeof(*size) < sizeof(unsigned long long)) && ((size64 * (blocksize / 512)) > 0xFFFFFFFF))
				return EFBIG;
...
int lfopen(const char *device, int flags)
{
...
int tivo_partition_validate(struct tivo_partition_table *table)
{
	uint64_t loop;
	char partsused[256];
 ...
 */
int tivo_partition_remap(const char *device, int old, int new)
{
	struct tivo_partition_table *table;
